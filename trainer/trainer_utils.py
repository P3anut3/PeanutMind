...
from model.model import PeanutMindForCausalLM
...
def init_model(
    ...
    tokenizer_path="../model",  # tokenizer 文件（tokenizer.json 等）直接放在 model/ 目录下
    save_dir="../out",
    device="cuda",
):
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_path)
    model = PeanutMindForCausalLM(lm_config)
...
def safe_math_eval(expression):
    ...
    def pow_guard(
        base, exp
    ):  # 幂运算统一走这里，拦截 9**9**9 / 10**99999 这类把进程算死的输入
    ...
    funcs = {
        ...
        **{
            n: getattr(math, n)
            for n in "sqrt exp log log2 log10 sin cos tan asin acos atan atan2 sinh cosh tanh floor ceil trunc fabs fmod hypot gcd degrees radians".split()
        },
    }
    bin_ops = {
